...
* Le champ **Adresses des clients sur liste noire** indique quels sont les adresses IP/DNS des clients
* Le champ **Entête HTTP SSO** indique la variable de l'en-tête qui sera utilisée comme login/pseudo.
* Le champ **Expression régulière SSO de recherche sur le login** indique l'expression régulière de recherche sur le nom d'utilisateur. Si vous souhaitez supprimer le domaine de l'adresse mail: /@.*/
* Le champ **Expression régulière SSO de remplacement sur le login** indique la chaine de caractères remplaçant l'expression régulière précédente.
