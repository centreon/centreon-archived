...
.. warning::
    In case of migration from CES 3.4.x, Centreon-web 2.8.x under CentOS 6 with MariaDB 5.X, do not sync folders /var/lib/mysql with RSYNC toward Centreon 19.04 / MariaDB 10.1.
    ...
    #. Export the dumps to the new Centreon 19.04 database server (make sure you have enough space for large databases dumps): ::
    
        # rsync -avz /tmp/centreon.sql root@IP_New_Centreon:/tmp/
        # rsync -avz /tmp/centreon_storage.sql root@IP_New_Centreon:/tmp/
        
    #. On the Centreon 19.04 database server, drop the original databases and create them again: ::
...
.. note::
    ...
    postpone the upgrade to v19.04, be aware that centreon-web 19.04 resource
    ...
    You can now push poller configuration from Centreon 19.04 whether the distant poller is centreon-engine 19.04 or 1.8.1
    
Upgrading Centreon
==================

On the new server, force the update by moving the contents of the
**/usr/share/centreon/installDir/install-19.04.0-YYYYMMDD_HHMMSS** directory to
...
with Centreon 19.04 and perform the upgrade.
