...
if ($rows != 0) {
    for ($i = $istart; $i <= $iend; $i++) {
        ...
        $urlPage = "./main.get.php?p=" . $p . "&num=$i&limit=" . $limit . "&poller=" . $poller .
    ...
    if (($prev = $num - 1) >= 0) {
        $tpl->assign(
            'pagePrev',
            ("./main.get.php?p=" . $p . "&num=$prev&limit=" . $limit . "&poller=" . $poller .
                "&template=$template&search=" . $search . "&type=" . $type . "&o=" . $o . $url_var)
        );
    }
    ...
    if (($next = $num + 1) < ($rows / $limit)) {
        $tpl->assign(
            'pageNext',
            ("./main.get.php?p=" . $p . "&num=$next&limit=" . $limit . "&poller=" . $poller .
                "&template=$template&search=" . $search . "&type=" . $type . "&o=" . $o . $url_var)
        );
    }
    ...
    if ($page_max > 5 && $num != 0) {
        $tpl->assign(
            'firstPage',
            ("./main.get.php?p=" . $p . "&num=0&limit=" . $limit . "&poller=" . $poller .
                "&template=$template&search=" . $search . "&type=" . $type . "&o=" . $o . $url_var)
        );
    }
    if ($page_max > 5 && $num != ($pageNumber - 1)) {
        $tpl->assign(
            'lastPage',
            ("./main.get.php?p=" . $p . "&num=" . ($pageNumber - 1) . "&limit=" . $limit .
