 ...
 */
	...
	if (isset($_GET["svc_group_id"]) && isset($_GET["color"]) && isset($_GET["today_ok"])&& isset($_GET["today_critical"]) && isset($_GET["today_unknown"])&& isset($_GET["today_pending"])){
		list($colorOK, $colorWARNING, $colorCRITICAL, $colorUNKNOWN, $colorUNDETERMINED)= split (":", $_GET["color"], 5);

		# Declaring connexion with Centreon database
		$dsn = array(
			     'phptype'  => 'mysql',
		...
		# Declaring connexion with CentStorage database
		$dsn = array(
			     'phptype'  => 'mysql',
			     'username' => $conf_centreon['user'],
			     'password' => $conf_centreon['password'],
		...
		if (PEAR::isError($pearDB)) 
		  	die("Connecting probems with centstorage database : " . $pearDB->getMessage());		
		...
		while ($h =& $res->fetchRow()) {
			...
			$bubultab .= '{tr}{td class=bulleDashleft style="background:#'.$colorUNKNOWN.';"  }Unknown:{/td}{td class=bulleDash}'. Duration::toString($unknowntime) .'{/td}{td class=bulleDash}'.(($punknown > 0) ? $punknown : "0").'%{/td}{td class=bulleDash}{/td}{/tr}';
			$bubultab .= '{tr}{td class=bulleDashleft style="background:#'.$colorUNDETERMINED.';"  }Undetermined:{/td}{td class=bulleDash}'. Duration::toString($pendingtime) .'{/td}{td class=bulleDash}'.(($ppending > 0) ? $ppending : "0").'%{/td}{/tr}';
			$bubultab .= '{/table}';
...
#
	...
	$bubultab .= '{tr}{td class=bulleDashleft style="background:#'.$colorUNDETERMINED.';"  }Undetermined:{/td}{td class=bulleDash}'.  Duration::toString($_GET["today_pending"] * $t / 100) .'{/td}{td class=bulleDash}'.(($_GET["today_pending"] > 0) ? $_GET["today_pending"] : "0").'%{/td}{/tr}';
	$bubultab .= '{/table}';
