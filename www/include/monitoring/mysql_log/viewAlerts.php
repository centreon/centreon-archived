...
*/
	...
	if (isset($_GET["o"]) && $_GET["o"] == "alerts_host"){
		...
		if (isset($_GET["search3"]) && isset($_GET["sort_type3"]) && $_GET["search3"] && $_GET["sort_type3"])
			...
			$sort_str3 = "";
		$DBRESULT =& $pearDBO->query("SELECT ctime,status,host_name,output,retry FROM log WHERE ctime > '$start' AND ctime <= '$end' AND msg_type = '1' ".$sort_str1.$sort_str2.$sort_str3."ORDER BY log_id DESC , ctime DESC");
		...
		if (isset($_GET["search3"]) && isset($_GET["sort_type3"]) && $_GET["search3"] && $_GET["sort_type3"])
			...
			$sort_str3 = "";
		$DBRESULT =& $pearDBO->query("SELECT ctime,status,host_name,service_description,output,retry FROM log WHERE ctime > '$start' AND ctime <= '$end' AND msg_type = '0' ".$sort_str1.$sort_str2.$sort_str3."ORDER BY log_id DESC , ctime DESC");
		if (PEAR::isError($DBRESULT))
			print "Mysql Error : ".$DBRESULT->getMessage();
	    for ($cpt = 0;$DBRESULT->fetchInto($log);$cpt++){
		...
		if (isset($_GET["search3"]) && isset($_GET["sort_type3"]) && $_GET["search3"] && $_GET["sort_type3"])
			...
			$sort_str3 = "";
		$DBRESULT =& $pearDBO->query("SELECT ctime,status,host_name,service_description,output,retry FROM log WHERE ctime > '$start' AND ctime <= '$end' AND msg_type <= '1' ".$sort_str1.$sort_str2.$sort_str3."ORDER BY log_id DESC , ctime DESC");
		if (PEAR::isError($DBRESULT))
			print "Mysql Error : ".$DBRESULT->getMessage();
	    for ($cpt = 0;$DBRESULT->fetchInto($log);$cpt++){
