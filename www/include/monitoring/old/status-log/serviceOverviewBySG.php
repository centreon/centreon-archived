		...
		while ($sg_relation =& $DBRESULT_relation->fetchRow()){
			if ($sg_relation["host_host_id"]){
				...
				if ($oreon->user->admin || !$isRestreint || ($isRestreint && isset($TabLca["LcaHost"][$host_name]))){
					...
					|| 	(!isset($_GET["problem"]) && isset($_GET["acknowledge"]) && $_GET["acknowledge"] == 1 && isset($service_status[$host_name."_".$service_description]) && $service_status[$host_name."_".$service_description]["problem_has_been_acknowledged"] == 1)
					|| 	(!isset($_GET["problem"]) && isset($_GET["acknowledge"]) && $_GET["acknowledge"] == 0 && isset($service_status[$host_name."_".$service_description]) && $service_status[$host_name."_".$service_description]["problem_has_been_acknowledged"] == 0 && $service_status[$host_name."_".$service_description]["current_state"] != "OK" )){			
