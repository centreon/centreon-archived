 ...
 */
	...
	$rq1 = 	"SELECT DISTINCT no.name1 as host_name, nh.address, nh.alias as host_alias, " .
			...
			" nh.notes_url, nh.notes, nh.icon_image, nh.address, i.instance_name " .
			" FROM ".$obj->ndoPrefix."hoststatus nhs, ".$obj->ndoPrefix."objects no, ".$obj->ndoPrefix."hosts nh, ".$obj->ndoPrefix."instances i ";
	if ($hostgroups) {
	...
	$rq1 .= " WHERE no.object_id = nhs.host_object_id " .
	        ...
	        "   AND no.instance_id = i.instance_id ";
	...
	$ArgNeeded = "A.host_name, A.object_id, A.service_description, A.notes, A.notes_url, A.action_url, A.max_check_attempts, A.icon_image,
				  A.display_name, A.instance_name, nss.process_performance_data, nss.current_state, nss.output as plugin_output,
	...
	$rq2 = 	"SELECT DISTINCT no.name1 as host_name, no.object_id, no.name2 as service_description, " .
			"ns.notes, ns.notes_url, ns.icon_image, ns.action_url, ns.max_check_attempts, ns.display_name, i.instance_name
			 FROM  ".$obj->ndoPrefix."objects no, ".$obj->ndoPrefix."services ns, ".$obj->ndoPrefix."instances i $ACLDBName" .
