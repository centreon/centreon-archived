 ...
 */
		...
		while ($DBRESULT_relation->fetchInto($sg_relation)){
			if ($sg_relation["host_host_id"]){
				...
				if ($oreon->user->admin || !$isRestreint || ($isRestreint && isset($TabLca["LcaHost"][$host_name]))){
					...
					if ((isset($_GET["problem"]) && isset($service_status[$host_name."_".$service_description]) && $service_status[$host_name."_".$service_description]["current_state"] != "OK") 
					|| 	(!isset($_GET["problem"]) && !isset($_GET["acknowledge"])) 
					|| 	(!isset($_GET["problem"]) && isset($_GET["acknowledge"]) && isset($service_status[$host_name."_".$service_description]) && $_GET["acknowledge"] == 1 && $service_status[$host_name."_".$service_description]["problem_has_been_acknowledged"] == 1)
					|| 	(!isset($_GET["problem"]) && isset($_GET["acknowledge"]) && isset($service_status[$host_name."_".$service_description]) && $_GET["acknowledge"] == 0 && $service_status[$host_name."_".$service_description]["problem_has_been_acknowledged"] == 0 && $service_status[$host_name."_".$service_description]["current_state"] != "OK" )){							
						if (isset($service_status[$host_name."_".$service_description])){	
							...
							$svc_data[$r["sg_name"]][$host_name] .= " <span style='background:".$oreon->optGen["color_".strtolower($service_status[$host_name."_".$service_description]["current_state"])]."'><a href='./main.php?p=202&o=svcd&host_name=".$host_name."&service_description=".$service_description."'>".$service_description."</a></span>&nbsp; \n";				
	...
	$tpl->display("serviceGridBySG.ihtml");
?>