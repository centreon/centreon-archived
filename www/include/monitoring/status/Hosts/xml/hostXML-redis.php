...
error_log("~@~@~@~@~@~@ DEFAULT CRITICALITY" 
            . print_r($obj->defaultCriticality, true));
/*
...
if ($statusHost == 'hpb' || $statusHost == 'h_unhandled') {
    if ($statusService == 'hpb') {
...
error_log('SORT TYPE: ' . $sort_type);
error_log('ORDER: ' . $order);
if (empty($sort_type) || $sort_type == 'name') {
    ...
    $get_params = array_merge($get_params,
            array('SORT', 2, $ty[$sort_type][0], $t, 'name', 'ALPHA'));
}
if (!empty($filter)) {
...
error_log('redis->rawCommand ' . print_r($get_params, true));
...
foreach ($rows as $row) {
    ...
    $obj->XML->writeElement('hc', $obj->colorHost[$row['current_state']]);
    $obj->XML->writeElement('f', $flag);
    $obj->XML->writeElement("hid", $row['host_id']);
    $obj->XML->writeElement("hn", CentreonUtils::escapeSecure($row['name']), false);
    $obj->XML->writeElement("hnl", CentreonUtils::escapeSecure(urlencode($row['name'])));
    $obj->XML->writeElement("a", ($row['address'] ? CentreonUtils::escapeSecure($row['address']) : 'N/A'));
    $obj->XML->writeElement('ou', ($row['plugin_output'] ? CentreonUtils::escapeSecure($row['plugin_output']) : 'N/A'));
    $obj->XML->writeElement('lc', ($row['last_check'] != 0 ? CentreonDuration::toString(time() - $row['last_check']) : 'N/A'));
    $obj->XML->writeElement("cs", _($obj->statusHost[$row['current_state']]), false);
    $obj->XML->writeElement("pha", $row['acknowledged']);
    $obj->XML->writeElement("pce", $row['passive_checks']);
    $obj->XML->writeElement("ace", $row['active_checks']);
    $obj->XML->writeElement("lsc", ($duration ? $duration : 'N/A'));
    $obj->XML->writeElement("lhs", ($hard_duration ? $hard_duration : "N/A"));
    $obj->XML->writeElement('ha', $row['acknowledged']);
    $obj->XML->writeElement('hdtm', $row['scheduled_downtime_depth']);
    $obj->XML->writeElement("hdtmXml", "./include/monitoring/downtime/xml/broker/makeXMLForDowntime.php?hid=".$row['host_id']);
    $obj->XML->writeElement("hdtmXsl", "./include/monitoring/downtime/xsl/popupForDowntime.xsl");
    $obj->XML->writeElement("hackXml", "./include/monitoring/acknowlegement/xml/broker/makeXMLForAck.php?hid=".$row['host_id']);
    $obj->XML->writeElement("hackXsl", "./include/monitoring/acknowlegement/xsl/popupForAck.xsl");
    $obj->XML->writeElement('hae', $row['active_checks']);
    $obj->XML->writeElement('hpe', $row['passive_checks']);
