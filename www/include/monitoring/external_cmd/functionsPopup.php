	...
	function write_command($cmd, $poller){
      ...
      */
		...
		if ($actions == true || $is_admin) {
			...
			$flg = write_command(" ACKNOWLEDGE_SVC_PROBLEM;".urldecode($host_name).";".urldecode($svc_description).";".$sticky.";".$notify.";".$persistent.";".$_GET["author"].";".$_GET["comment"], $host_poller);
			if ($force_check == 1) {
				write_command(" SCHEDULE_FORCED_SVC_CHECK;".urldecode($host_name).";".urldecode($svc_description).";".time(), $host_poller);
			}
			set_user_param($oreon->user->user_id, $pearDB, "ack_sticky", $sticky);
			set_user_param($oreon->user->user_id, $pearDB, "ack_notify", $notify);
			set_user_param($oreon->user->user_id, $pearDB, "ack_persistent", $persistent);
