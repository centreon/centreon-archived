...
*/
	...
	$DBRESULT =& $pearDBO->query("SELECT * FROM `index_data` WHERE id = '".$_GET["index_id"]."'");
	if (PEAR::isError($DBRESULT))
	...
	for ($i = 0;$DBRESULT->fetchInto($index_data);$i++){
		...
		for ($im = 0;$DBRESULT2->fetchInto($metrics);$im++){
			...
			if (isset($metrics["unit_name"]) && $metrics["unit_name"])
				$metric .= " (".$metrics["unit_name"].") <br>";
		}
		$index_data["metrics_name"] = $metric;
		$index_data["service_id"] = $index_data["id"];
	...
	$tpl->display("viewServices.ihtml");
?>