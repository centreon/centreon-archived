...
if (isset($_GET["o"]) && $_GET["o"] == "k" && $userId !== false) {
    $statement = $pearDB->prepare("DELETE FROM session WHERE user_id = ?");
    $pearDB->execute($statement, [$userId]);
    $msg = new CentreonMsg();
...
for ($cpt = 0; $r = $res->fetchRow(); $cpt++) {
    ...
    if ($centreon->user->admin) {
        $session_data[$cpt]["actions"] = "<a href='./main.php?p=$p&o=k&user=" . $r['user_id'] . "'><img src='./img/icons/delete.png' border='0' alt='"._("Kick User")."' title='"._("Kick User")."'></a>";
    } else {
