...
$DBRESULT = $pearDB->query("SELECT acl_topo_id, acl_topo_name, acl_topo_alias, acl_topo_activate
...
for ($i = 0; $topo = $DBRESULT->fetchRow(); $i++) {
    ...
    if ($topo["acl_topo_activate"]) {
        $moptions = "<a href='main.php?p=".$p."&acl_topo_id=".$topo['acl_topo_id']
    ...
    $moptions .= "<input onKeypress=\"if(event.keyCode > 31 && (event.keyCode < 45 || event.keyCode > 57)) "
        ...
        . "name='dupNbr[".$topo['acl_topo_id']."]'></input>";

    /* Contacts */
    $ctNbr = array();
    $DBRESULT2 = $pearDB->query("SELECT COUNT(*) AS nbr FROM acl_topology_relations
        WHERE acl_topo_id = '".$topo['acl_topo_id']."'");
