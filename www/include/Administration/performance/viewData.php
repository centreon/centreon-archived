    ...
    if ($inputs["o"] == "rg" && !empty($selected)) {
        ...
        foreach ($selected as $key => $value) {
            $DBRESULT = $pearDBO->query(
                "UPDATE index_data SET `must_be_rebuild` = '0' WHERE id = '" .
        ...
        if (count($listMetricsToDelete) > 0) {
            $pearDBO->query("UPDATE metrics SET to_delete = 1 WHERE metric_id IN (" .
                join(', ', $listMetricsToDelete) . ")");
            $pearDBO->query("UPDATE index_data SET to_delete = 1 WHERE id IN (" .
                join(', ', array_keys($selected)) . ")");
...
if (isset($inputs["o"]) && $inputs["o"] == "d" && isset($inputs["id"])) {
    $DBRESULT = $pearDBO->query(
        "UPDATE index_data SET `trashed` = '1' WHERE id = '" .
...
if (isset($inputs["o"]) && $inputs["o"] == "rb" && isset($inputs["id"])) {
    $DBRESULT = $pearDBO->query(
        "UPDATE index_data SET `must_be_rebuild` = '1' WHERE id = '" .
        htmlentities($inputs["id"], ENT_QUOTES, 'UTF-8') . "'"
    );
}
...
$data = array();
$DBRESULT = $pearDBO->query(
    "SELECT SQL_CALC_FOUND_ROWS DISTINCT i.* FROM index_data i, metrics m" . $extTables .
    " WHERE i.id = m.index_id $search_string ORDER BY host_name, service_description LIMIT " .
    $num * $limit . ", $limit"
);
...
for ($i = 0; $index_data = $DBRESULT->fetchRow(); $i++) {
    $DBRESULT2 = $pearDBO->query(
        "SELECT * FROM metrics WHERE index_id = '" . $index_data["id"] . "' ORDER BY metric_name"
    );
    $metric = "";
    ...
    $index_data["service_description"] = "<a href='./main.php?p=50119&o=msvc&index_id=" .
        $index_data["id"] . "'>" . $index_data["service_description"] . "</a>";
