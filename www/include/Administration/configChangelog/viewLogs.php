        ...
        if ($res['object_id']) {
            ...
            if ($res['object_type'] == "service") {
                if (isset($host_name) && $host_name != '') {
                    ...
                    //we are going to purge the logs, 
                    //if we don't do that it will leaves blank pages in the pagination
                    if ( empty($contactList[$res['log_contact_id']]) || $contactList[$res['log_contact_id']] == "" ) {
                        //it looks like that sometimes the author is empty
                        //from what i've seen, it's due to a CLAPI action
