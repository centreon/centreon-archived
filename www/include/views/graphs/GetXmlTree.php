...
if ($normal_mode) {
    ...
    if ($type == "HG") {
        ...
        if ($search != "")
            $DBRESULT = $pearDB->query("SELECT DISTINCT * FROM servicegroup WHERE `sg_name` LIKE '%".CentreonDB::escape($search)."%' ORDER BY `sg_name`");
        else
            ...
            $rq = "SELECT hg_id, hg_name
					   ...
					   AND (hg_id IN
					   	    	...
					   	    	OR hg_id IN
					   	    		...
					   	    		(host.host_name LIKE '%".CentreonDB::escape($search)."%' OR `host_alias` LIKE '%".CentreonDB::escape($search)."%')))
        ...
        $searchSTR = "";
        if ($search != "")
            $searchSTR = " AND (`host_name` LIKE '%".CentreonDB::escape($search)."%' OR `host_alias` LIKE '%".CentreonDB::escape($search)."%') ";
