 ...
 */
	...
	(isset($_GET["warning"]))           ? $warning = htmlentities($_GET["warning"], ENT_QUOTES, "UTF-8") : $warning = "0";
	(isset($_GET["critical"]))          ? $critical = htmlentities($_GET["critical"], ENT_QUOTES, "UTF-8") : $critical = "0";
	(isset($_GET["StartDate"]))         ? $StartDate = htmlentities($_GET["StartDate"], ENT_QUOTES, "UTF-8") : $StartDate = "";
	(isset($_GET["EndDate"]))           ? $EndDate = htmlentities($_GET["EndDate"], ENT_QUOTES, "UTF-8") : $EndDate = "";
	...
	foreach ($tab_real_id as $key => $openid) {
		...
		if ($type == "HS" || $type == "MS"){
			...
			if ($type == "HS"){
				...
				$DBRESULT2 = $pearDBO->query("SELECT i.id, i.service_id, i.service_description, i.host_name FROM index_data i, metrics m WHERE i.id = m.index_id AND i.trashed = '0' AND i.host_id = '".$tab_tmp[2]."' AND i.service_id = '".$tab_tmp[1]."'");
			...
			$dbindd = $pearDBO->query("SELECT host_id, service_id FROM index_data i, metrics m WHERE i.id = m.index_id AND id = '".$svc_id["id"]."'");
