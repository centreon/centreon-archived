		...
		for ($i = 1; $i <= $nbrDup[$key]; $i++)	{
			...
			if (isset($esc_name) && testExistence($esc_name))	{
				...
				if (isset($maxId["MAX(esc_id)"]))	{
					...
					$centreon->CentreonLogAction->insertLog("escalation", $maxId["MAX(esc_id)"], $esc_name, "a");
				}
			}
		}
	}
...
function insertEscalation()	{
	...
	$centreon->CentreonLogAction->insertLog("escalation", $esc_id["MAX(esc_id)"], htmlentities($ret["esc_name"], ENT_QUOTES, "UTF-8"), "a", $fields);
...
function updateEscalation($esc_id = null)	{
	...
	if (isset($ret["esc_metas"]))
		$fields["esc_metas"] = implode(",", $ret["esc_metas"]);
	$centreon->CentreonLogAction->insertLog("escalation", $esc_id["MAX(esc_id)"], htmlentities($ret["esc_name"], ENT_QUOTES, "UTF-8"), "c", $fields);
}
