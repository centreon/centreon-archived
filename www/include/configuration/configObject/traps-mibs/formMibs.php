 ...
 */
	...
	if ($form->validate())	{
		...
		if ($fileObj->isUploadedFile()) {
			...
			if ($debug)
				print("@CENTPLUGINSTRAPS_BINDIR@/centFillTrapDB -f /tmp/".$values["name"].".conf -m ".htmlentities($ret["mnftr"], ENT_QUOTES, "UTF-8")." 2>&1");

			$stdout = shell_exec("@CENTPLUGINSTRAPS_BINDIR@/centFillTrapDB -f /tmp/".$values["name"].".conf -m ".htmlentities($ret["mnftr"], ENT_QUOTES, "UTF-8")." 2>&1");
