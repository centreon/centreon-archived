 ...
 */
$DBRESULT = $pearDB->query("SELECT * FROM traps_group $SearchTool ORDER BY traps_group_name LIMIT ".$num * $limit.", ".$limit);
...
for ($i = 0; $group = $DBRESULT->fetchRow(); $i++) {
    ...
    $moptions = "&nbsp;<input onKeypress=\"if(event.keyCode > 31 && (event.keyCode < 45 || event.keyCode > 57)) event.returnValue = false; if(event.which > 31 && (event.which < 45 || event.which > 57)) return false;\" maxlength=\"3\" size=\"3\" value='1' style=\"margin-bottom:0px;\" name='dupNbr[".$group['traps_group_id']."]'></input>";
