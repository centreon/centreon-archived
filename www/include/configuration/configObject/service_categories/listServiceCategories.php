...
$tpl->assign("headerMenu_linked_svcTpl", _("Number of linked services Templates"));
...
for ($i = 0; $sc = $DBRESULT->fetchRow(); $i++) {
    ...
    $DBRESULT2 = $pearDB->query("SELECT COUNT(*) FROM service_categories_relation, service WHERE sc_id = '".$sc['sc_id']."' AND service_service_id = service_id AND service.service_register = '1'");
    $nb_svc = $DBRESULT2->fetchRow();

    $DBRESULT2 = $pearDB->query("SELECT COUNT(*) FROM service_categories_relation, service WHERE sc_id = '".$sc['sc_id']."' AND service_service_id = service_id AND service.service_register = '0'");
    $nb_svcTpl = $DBRESULT2->fetchRow();
