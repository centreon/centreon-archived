 ...
 */
	...
	function insertHostDependency($ret = array())	{
		...
		$oreon->CentreonLogAction->insertLog("host dependency", $dep_id["MAX(dep_id)"], htmlentities($ret["dep_name"], ENT_QUOTES, "UTF-8"), "a", $fields);
	...
	function updateHostDependency($dep_id = null)	{
		...
		if (isset($ret["dep_hostChilds"]))
			$fields["dep_hostChilds"] = implode(",", $ret["dep_hostChilds"]);
		$oreon->CentreonLogAction->insertLog("host dependency", $dep_id, htmlentities($ret["dep_name"], ENT_QUOTES, "UTF-8"), "c", $fields);
	}
