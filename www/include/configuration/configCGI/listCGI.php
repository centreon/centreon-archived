 ...
 */
    ...
    if (!$oreon->user->admin && count($allowedCgiConf)) {
        ...
        $aclCond .= "cgi_id IN (".implode(',', array_keys($allowedCgiConf)).") ";
    }

	$DBRESULT = $pearDB->query("SELECT COUNT(*) FROM cfg_cgi $SearchTool $aclCond");
	 ...
	 * Nagios list
	 */
	$rq = "SELECT cgi_id, cgi_name, cgi_comment, cgi_activate
           FROM cfg_cgi $SearchTool $aclCond
           ORDER BY cgi_name
	...
	for ($i = 0; $cgi = $DBRESULT->fetchRow(); $i++) {
		...
		if ($cgi["cgi_activate"])
			$moptions .= "<a href='main.php?p=".$p."&cgi_id=".$cgi['cgi_id']."&o=u&limit=".$limit."&num=".$num."&search=".$search."'><img src='img/icons/eye_inactive.png' class='ico-14' border='0' alt='"._("Disabled")."'></a>&nbsp;&nbsp;";
		else
			$moptions .= "<a href='main.php?p=".$p."&cgi_id=".$cgi['cgi_id']."&o=s&limit=".$limit."&num=".$num."&search=".$search."'><img src='img/icons/eye_active.png' class='ico-14' border='0' alt='"._("Enabled")."'></a>&nbsp;&nbsp;";
