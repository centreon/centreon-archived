 ...
 */
	...
	$xml->writeElement('database_db2', _('Database DB2'));
	$xml->writeElement('database_mysql', _('Database MySQL'));
	$xml->writeElement('database_odbc', _('Database ODBC'));
	$xml->writeElement('database_oracle', _('Database Oracle'));
	$xml->writeElement('database_postgres', _('Database Postgres'));
	$xml->writeElement('database_sqlite', _('Database sqlite'));
	$xml->writeElement('database_tds', _('Database TDS'));
	...
	$xml->writeElement('dbname', _('Database name'));
	$xml->writeElement('username', _('Username'));
	$xml->writeElement('password', _('Password'));
	$xml->endElement(); /* lang */
	$xml->endElement(); /* main */

	if (isset($_GET['config_id']) && $_GET['config_id'] != 0) {
	    $query = "SELECT config_key, config_value, config_group_id FROM cfg_centreonbroker_info WHERE config_id = " . $_GET['config_id'] . " AND config_group = 'output' ORDER BY config_group_id";
	    $res = $db->query($query);
	    if (!PEAR::isError($res)) {
	        $infos = array();
