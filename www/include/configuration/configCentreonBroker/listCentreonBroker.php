 ...
 */
	...
	if (isset($search)) {
		$DBRESULT = & $pearDB->query("SELECT COUNT(*) FROM `cfg_centreonbroker` WHERE config_name LIKE '%".htmlentities($search, ENT_QUOTES, "UTF-8")."%'");
	} else {
		$DBRESULT = & $pearDB->query("SELECT COUNT(*) FROM `cfg_centreonbroker`");
	}
	 ...
	 */
	if ($search)
		$rq = "SELECT config_id, config_name, ns_nagios_server, config_activate FROM cfg_centreonbroker WHERE description LIKE '%".htmlentities($search, ENT_QUOTES, "UTF-8")."%' ORDER BY config_name LIMIT ".$num * $limit.", ".$limit;
	else
		$rq = "SELECT config_id, config_name, ns_nagios_server, config_activate FROM cfg_centreonbroker  ORDER BY config_name LIMIT ".$num * $limit.", ".$limit;
