 ...
 */
    ...
    $aclCond = "";
    if (!$oreon->user->admin && count($allowedNdo2db)) {
        if (isset($search) && $search) {
            $aclCond = " AND ";
        } else {
            $aclCond = " WHERE ";
        }
        $aclCond .= "id IN (".implode(',', array_keys($allowedNdo2db)).") ";
    }
	 ...
	 */
	if ($search) {
		$rq = "SELECT id, description, ns_nagios_server, activate
               FROM cfg_ndo2db
               WHERE description
               LIKE '%".htmlentities($search, ENT_QUOTES, "UTF-8")."%'
               $aclCond
