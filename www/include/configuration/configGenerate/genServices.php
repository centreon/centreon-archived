...
$critSCache = array();
$CritSData = array();
$critRes = $pearDB->query("SELECT scr.sc_id, scr.service_service_id, sc.sc_name, level, sc.sc_id
...
while ($critRow = $critRes->fetchRow()) {
    $critSCache[$critRow['service_service_id']] = $critRow['sc_id'];
}

/*
 * Get Criticities In cache
 */
$critRes = $pearDB->query("SELECT * FROM service_categories WHERE level IS NOT NULL ORDER BY level DESC");
while ($critRow = $critRes->fetchRow()) {
    $critSData[$critRow["sc_name"]] = array("name" => $critRow['sc_name'], "id" => $critRow['sc_id'], "level" => $critRow["level"]);
}
...
if ($oreon->CentreonGMT->used() == 1) {
    foreach ($hostGenerated as $host_id => $host_name) {
        ...
        foreach ($svcList as $svc_id => $svc_name) {
             ...
             * Criticality level
             */
            if (isset($critSCache[$service['service_id']])) {
                $critDataTmp = $criticality->getData($critSCache[$service['service_id']], true);
                if (!is_null($critDataTmp)) {
                    $strTMP .= print_line(";_CRITICALITY_NAME", $critDataTmp['name']);
                ...
                if ($buff == 0) {
                    ...
                    if (isset($critCacheName[$host_name])) {
                        ...
                        $strTMP .= print_line(";_CRITICALITY_NAME", $critSData[$name]['name']);
                        ...
                        $strTMP .= print_line(";_CRITICALITY_NAME", $critSData[$tmp1['name']]['name']);
