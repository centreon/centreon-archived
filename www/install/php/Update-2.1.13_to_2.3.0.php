...
if (isset($pearDB)) {
    ...
    while ($rows = $res->fetchRow()) {
        $query2 = "UPDATE command SET command_name = '" .
            upgradeReplaceSpecialChars(html_entity_decode($rows['command_name'])) . "',
        		   ...
        		   command_example = '" .
            upgradeReplaceSpecialChars(html_entity_decode($rows['command_example'])) .
    ...
    while ($rows = $res->fetchRow()) {
        $query2 = "UPDATE hostgroup SET hg_name = '" .
            upgradeReplaceSpecialChars(html_entity_decode($rows['hg_name'])) . "',
        		   hg_alias = '" .
            upgradeReplaceSpecialChars(html_entity_decode($rows['hg_alias'])) .
    ...
    $res = $pearDB->query($query);
    while ($rows = $res->fetchRow()) {
        $query2 = "UPDATE on_demand_macro_service SET svc_macro_value = '" .
