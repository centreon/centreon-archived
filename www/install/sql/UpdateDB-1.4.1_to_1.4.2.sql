...
) ENGINE = innodb COMMENT = 'Services Categories For best Reporting';
...
UPDATE `topology` SET `topology_name` = 'menu_ODS_config',`topology_parent` = '501', `topology_page` = '5010601', `topology_order` = '5' WHERE `topology`.`topology_page` = 5010110;
...
ALTER TABLE `escalation_servicegroup_relation`  ADD CONSTRAINT `escalation_servicegroup_relation_ibfk_1` FOREIGN KEY (`escalation_esc_id`) REFERENCES `escalation` (`esc_id`) ON DELETE CASCADE, ADD CONSTRAINT `escalation_servicegroup_relation_ibfk_2` FOREIGN KEY (`servicegroup_sg_id`) REFERENCES `servicegroup` (`sg_id`) ON DELETE CASCADE;
