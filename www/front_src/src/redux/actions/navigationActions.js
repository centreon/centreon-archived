...
export const fetchReactRoutesData = () => {
  return async (dispatch) => {
    try {
      ...
      dispatch(fetchReactRoutesSuccess(data.result));
    } catch (err) {
      console.error(err);
    }
  };
...
export const fetchNavigationData = () => {
  return async (dispatch) => {
    ...
    try {
      ...
      const pageIds = [];
      for (const levelOne of data.result) {
        pageIds.push(levelOne.page);
        for (const levelTwo of levelOne.children) {
          pageIds.push(levelTwo.page);
          for (const group of levelTwo.groups) {
            for (const levelThree of group.children) {
...
export const setNavigation = (data) => {
  ...
  for (const [levelOneKey, levelOneProps] of Object.entries(data)) {
    navigationData.push(levelOneKey.slice(1));
    for (const [levelTwoKey, levelTwoProps] of Object.entries(
      levelOneProps.children,
    )) {
      navigationData.push(levelTwoKey.slice(1));
      for (const levelThreeProps of Object.values(levelTwoProps.children)) {
        for (const levelFourKey of Object.keys(levelThreeProps)) {
