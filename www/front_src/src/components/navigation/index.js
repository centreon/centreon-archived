...
class NavigationComponent extends Component {
  ...
  UNSAFE_componentWillMount = () => {
    this.getNavigation();
    document.addEventListener(`reloadNavigation`, this.getNavigation);
  };
  ...
  handleDoubleClick = (levelOneKey, levelOneProps) => {
    ...
    this.goToPage(routeMap.module + "?p=" + urlOptions, levelOneKey);
  };
    ...
    this.clickTimeout = setTimeout(() => {
      if (!this.doubleClicked) {
        ...
        Object.keys(menuItems).forEach(key => {
          menuItems[key].toggled =
            key === index ? !menuItems[index].toggled : false;
        });
  ...
  collapseLevelThree = (levelOneKey, levelTwoKey) => {
    ...
    Object.keys(menuItems[levelOneKey].children).forEach(subKey => {
      if (subKey === levelTwoKey) {
        menuItems[levelOneKey].children[subKey]["collapsed"] = !menuItems[
          levelOneKey
        ].children[subKey]["collapsed"];
      } else {
            ...
            {Object.entries(menuItems).map(([levelOneKey, levelOneProps]) =>
              levelOneProps.label ? (
                ...
                >
                  <span
                    onDoubleClick={() => {
                      this.handleDoubleClick(levelOneKey, levelOneProps);
                    }}
                    onClick={() => {
                      this.collapseLevelTwo(levelOneKey);
                    }}
                  ...
                  >
                    <span
                      class={`iconmoon icon-${levelOneProps.menu_id.toLowerCase()}`}
                    >
                  ...
                  <ul
                    ...
                    style={{
                      display:
                        levelOneProps.toggled && active ? "block" : "none"
                    }}
                  >
                    {Object.entries(levelOneProps.children).map(
                      ([levelTwoKey, levelTwoProps]) => {
                        const urlOptions =
                          levelTwoKey.slice(1) +
                          (levelTwoProps.options !== null
                            ? levelTwoProps.options
                            : "");
                        if (levelTwoProps.label) {
                          return (
                            <li
                              class={
                                "collapsed-item" +
                                (levelTwoProps.collapsed || pageId == urlOptions
                                  ? " active"
                                  : "")
                              }
                            >
                              {Object.keys(levelTwoProps.children).length >
                              0 ? (
                                <span
                                  style={{ cursor: "pointer" }}
                                  onClick={() => {
                                    this.collapseLevelThree(
                                      levelOneKey,
                                      levelTwoKey
                                    );
                                  }}
                                ...
                                >
                                  <Translate
                                    value={
                                      levelTwoProps.hasOwnProperty("label")
                                        ? levelTwoProps.label
                                        : ""
                                    }
                                  />
                              ...
                              <ul class="collapse-level collapsed-level-items first-level list-unstyled">
                                {Object.entries(levelTwoProps.children).map(
                                  ([levelThreeKey, levelThreeProps]) => {
                                    return (
                                      <React.Fragment>
                                        {Object.keys(levelTwoProps.children)
                                          .length > 1 && (
                                          <span class="collapsed-level-title">
                                            <Translate value={levelThreeKey} />
                                          </span>
                                        )}
                                        {Object.entries(levelThreeProps).map(
                                          ([levelFourKey, levelFourProps]) => {
                                            const urlOptions =
                                              levelFourKey.slice(1) +
                                              (levelFourProps.options !== null
                                                ? levelFourProps.options
                                                : "");
