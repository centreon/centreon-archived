...
class CentreonConfigPoller
{
    ...
    public function pollerReload($variables)
    {
        ...
        if (isset($host['localhost']) && $host['localhost'] == 1) {
            $msg_restart = exec(($host['use_sudo']=='1'?"sudo ":"") . $nagios_init_script . " reload", $stdout, $return_code);
        } else {
    ...
    public function pollerRestart($variables)
    {
        ...
        $msg_restart = "";
        if (isset($host['localhost']) && $host['localhost'] == 1) {
            $msg_restart = exec(escapeshellcmd(($host['use_sudo']=='1'?"sudo ":"") . $nagios_init_script . " restart"), $lines, $return_code);
        } else {
    ...
    public function pollerGenerate($variables, $login, $password)
    {
        ...
        if ($apacheUser != "") {
            ...
            foreach (glob($this->centreon_path."/filesGeneration/broker/$poller_id/*.{xml,cfg}", GLOB_BRACE) as $file) {
                chown($file, $apacheUser);
                chgrp($file, $apacheUser);
            }
        } else {
