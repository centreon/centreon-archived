...
class CentreonCustomView
{
    ...
    {
        $views = $this->getCustomViews();
        if (!isset($views[$viewId]) || ($views[$viewId]['deleteable'] == 0)) {
            return false;
        }
        return true;
    }
    ...
    public function getCustomViews()
    {
        if (!isset($this->customViews)) {
            $query = "SELECT cv.custom_view_id, name, layout, is_owner, locked, deleteable, user_id, usergroup_id, public
            ...
            while ($row = $res->fetchRow()) {
                ...
                if (!isset($tmp[$cvid]['deleteable']) || !$tmp[$cvid]['deleteable'] || $row['user_id']) {
                    $tmp[$cvid]['deleteable'] = $row['deleteable'];
                }
    ...
    public function addCustomView($params)
    {
        ...
        $query = "INSERT INTO custom_view_user_relation (custom_view_id, user_id, locked, deleteable, is_owner)
    ...
    public function loadCustomView($params)
    {
        if (isset($params['viewLoad']) && is_numeric($params['viewLoad']) && $params['viewLoad'] != -1) {
            $isOwner = 0;
            $res = $this->db->query("SELECT custom_view_id, user_id, locked, is_owner, is_consumed
            ...
            $this->db->query("INSERT INTO custom_view_user_relation
                (custom_view_id,user_id,is_owner,locked)
            ...
            $this->db->query("UPDATE custom_view_user_relation SET is_consumed = 1
    ...
    public function shareCustomView($params)
    {
        if ($this->checkPermission($params['custom_view_id'])) {
            ...
            if (isset($params['user_id']) && is_array($params['user_id'])) {
                foreach ($params['user_id'] as $userId) {
                    ...
                    $str .= "(" . $params['custom_view_id'] . ", " . $userId . ", " .
                        $params['locked']['locked'] . ", " . $params['deleteable']['deleteable'] . ", 0)";
            ...
            if ($str != "") {
                $this->db->query(
                    "REPLACE INTO custom_view_user_relation
                        (custom_view_id, user_id, locked, deleteable, is_consumed) VALUES " . $str
                );
            }
            ...
            if ($str != "") {
                $this->db->query(
                    "REPLACE INTO custom_view_user_relation
                        (custom_view_id, usergroup_id, locked, deleteable, is_consumed) VALUES " . $str
                );
            }
        }
    }
    ...
    public function getUsersFromViewId($viewId)
    {
        ...
        if (!isset($userList)) {
            $userList = array();
            $query = "SELECT contact_name, user_id, locked, deleteable
            ...
            while ($row = $res->fetchRow()) {
                ...
                $userList[$row['user_id']]['deleteable'] = $row['deleteable'];
            }
        }
        return $userList;
    }
    ...
    public function getUsergroupsFromViewId($viewId)
    {
        ...
        if (!isset($usergroupList)) {
            $usergroupList = array();
            $query = "SELECT cg_name, usergroup_id, locked, deleteable
            ...
            while ($row = $res->fetchRow()) {
                ...
                $usergroupList[$row['usergroup_id']]['deleteable'] = $row['deleteable'];
            }
        }
