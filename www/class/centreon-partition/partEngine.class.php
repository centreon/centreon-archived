...
class PartEngine {
    ...
    private function updateAddDailyPartitions($db, $tableName, $month, $day, $year) {
        ...
        print "[".date(DATE_RFC822)."][updateParts] Create new part : " . ($ntime[5] + 1900) . $month . $day . " - Range: $current_time\n";
        $request = "ALTER TABLE ".$tableName;
        $request .= " ADD PARTITION (PARTITION `p" . ($ntime[5] + 1900) . $month . $day . "` VALUES LESS THAN(" . $current_time . "))";
        $DBRESULT = $db->query($request);
        if (PEAR::isError($DBRESULT)) {
            throw new Exception("Error: cannot add a new partition 'p" . ($ntime[5] + 1900) . $month . $day . "' for table ".$tableName.", ".$DBRESULT->getDebugInfo()."\n");
        }
        return $current_time;
    }
	...
	private function getLastPartRange($table, $db) {
		...
		if (PEAR::isError($DBRESULT) || !$DBRESULT->numRows()) {
			throw new Exception("Error: cannot get table ".$table->getSchema().".".$table->getName()." last partition range, ".
	...
	public function purgeParts($table, $db) {
		...
		$DBRESULT = $db->query($request);
		if (PEAR::isError($DBRESULT)) {
			throw new Exception("Error : Cannot get partitions to purge for table ".$tableName.", ".$DBRESULT->getDebugInfo()."\n");	
        }
