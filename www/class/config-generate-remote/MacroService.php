...
class MacroService extends AbstractObject
{
    ...
    private function writeMacrosService($serviceId)
    {
        if ($this->checkGenerate($serviceId)) {
            return null;
        }

        foreach ($this->macroServiceCache[$serviceId] as $svc_macro_id => $value) {
            $this->generateObjectInFile($value, $serviceId);
        }
    }

    public function getServiceMacroByServiceId($serviceId)
    {
        # Get from the cache
