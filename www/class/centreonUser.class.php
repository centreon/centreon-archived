...
class CentreonUser
{
    ...
    public function setContactParameters($db, $parameters = array())
    {
        ...
        $deleteQuery = 'DELETE FROM contact_param WHERE cp_contact_id = :cp_contact_id AND  cp_key IN( :cp_key )';
        $queryKeys = implode('","', $keys);
        $stmt = $db->prepare($deleteQuery);
        $stmt->bindParam(':cp_contact_id', $this->user_id, PDO::PARAM_INT);
        $stmt->bindParam(':cp_key', $queryKeys, PDO::PARAM_STR);
