...
#
sub get_hostinfos($$$) {
    my $sth = $_[0]->prepare("SELECT host_name
    	FROM host
    	WHERE host_address='" . $_[1] . "' OR host_address='" . $_[2] . "'");
    $sth->execute();
    my @host;
...
sub get_hostlocation($$) {
    my $sth = $_[0]->prepare("SELECT localhost
...
sub get_hostNagiosServerID($$) {
    my $sth =
      $_[0]->prepare("SELECT id
      	FROM host, `ns_host_relation`, nagios_server
      	WHERE host.host_id = ns_host_relation.host_host_id
      		AND ns_host_relation.nagios_server_id = nagios_server.id
      		AND (host.host_name = '" . $_[1] . "'
      		OR host.host_address = '" . $_[1] . "')");
    $sth->execute();
    if ( $sth->rows() ) {
        my $temp = $sth->fetchrow_array();
        $sth->finish();
...
sub getServicesIncludeTemplate($$$$) {
    ...
    while ( my @temp = $sth_st->fetchrow_array() ) {
        my $tr_query = "SELECT `traps_id`
        	FROM `traps_service_relation`
        	WHERE `service_id` = '" . $temp[0] . "'
        		AND `traps_id` = '" . $trap_id . "'";
        ...
        if ( defined( $trap[0] ) ) {
            ...
            if ( defined( $temp[2] ) ) {
                ...
                while ( !$found ) {
                    my $st1_query = "SELECT `service_id`, `service_template_model_stm_id`, `service_description`
                    	FROM service s
                    	WHERE `service_id` = '" . $service_template . "'";
                    my $sth_st1 = $dbh->prepare($st1_query);
                    $sth_st1->execute();
                    my @st1_result = $sth_st1->fetchrow_array();
                    if ( defined( $st1_result[0] ) ) {
                        my $sth_st2 = $dbh->prepare("SELECT `traps_id`
                        	FROM `traps_service_relation`
                        	WHERE `service_id` = '" . $service_template . "'
                        		AND `traps_id` = '" . $trap_id . "'" );
                        $sth_st2->execute();
    ...
    foreach (@host) {
        ...
        foreach (@traps) {
	        ...
	        foreach (@servicename) {
	            ...
	            if ( defined($traps_advanced_treatment)
	                && $traps_advanced_treatment eq 1 )
	            {
	
	                # Check matching options
	                my $sth = $dbh->prepare("SELECT tmo_regexp, tmo_status, tmo_string FROM traps_matching_properties WHERE trap_id = '"
	                      . $trap_id
	                      . "' ORDER BY tmo_order" );
	                $sth->execute();
	                while ( my ( $regexp, $tmoStatus, $tmoString ) =
	                    $sth->fetchrow_array() )
	                {
	            ...
	            if ( defined($traps_submit_result_enable)
	                && $traps_submit_result_enable eq 1 )
	            {
	            ...
	            if ( defined($traps_reschedule_svc_enable)
	                && $traps_reschedule_svc_enable eq 1 )
	            {
	                my $id = get_hostNagiosServerID( $dbh, $this_host );
	                if ( defined($id) && $id != 0 ) {
	            ...
	            if ( defined($traps_execution_command_enable)
	                && $traps_execution_command_enable )
	            {
	                ...
	                if ( $htmlentities == 1 ) {
	                    $traps_execution_command =
	                      decode_entities($traps_execution_command);
	                }
	                else {
