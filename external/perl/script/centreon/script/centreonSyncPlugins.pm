...
sub run {
    ...
    ($status, $sth) = $cdb->query("SELECT `id`, `ip_address` FROM `nagios_server` WHERE `activate` = '1' AND `localhost` = '0'");
    die("Error SQL Quit") if ($status == -1);
    while ((my $data = $sth->fetchrow_hashref())) {
		my $ls = `$self->{ssh} -q $data->{'ip_address'} ls -l $path_plugins/ 2>> /dev/null | wc -l`;
        if ($ls > 1) {
            `$self->{rsync} -prc $path_plugins/* $data->{'ip_address'}:$path_plugins/`;
        } else {
            $self->{logger}->writeLogError("Directory not present on remote server : " . $data->{'ip_address'});
        }
	}
}
