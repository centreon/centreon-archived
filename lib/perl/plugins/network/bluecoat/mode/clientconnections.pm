...
sub run {
    ...
    my ($exit, $result) = $self->{snmp}->get_leef(oids => ['.1.3.6.1.4.1.3417.2.11.3.1.3.1.0', 
    ...
    $exit = $self->{perfdata}->threshold_check(value => $client_connections_active, threshold => [ { label => 'critical', 'exit_litteral' => 'critical' }, { label => 'warning', exit_litteral => 'warning' } ]);
