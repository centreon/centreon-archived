...
sub run {
    ...
    # Skipped disks
        ...
        $exit = $self->{perfdata}->threshold_check(value => $threshold_value, threshold => [ { label => 'warning', 'exit_litteral' => 'warning' }, { label => 'critical', exit_litteral => 'critical' } ]);
