...
<tr style='height:18px;' class='centreon-border-status-s-{$item["state_id"]}' role='row'>
    <td><span><a href='/centreon-realtime/host/{$item['host_id']}' target='_parent'><i class='fa fa-hdd-o'></i> {$item['name']}</a></span></td>
    <td><span><a href='/centreon-realtime/service/{$item['service_id']}' target='_parent'>{$item['description']}</a><span></td>
    <td>{$item['status']}</td>
    <td style='text-align:center;'>{$item['duration']}</td>
</tr>
{/foreach}
<tr>
    <td colspan='4' style='text-align:center;'><a href='../centreon-realtime/incident' target='_parent'>See more incidents</a></td>
</tr>
</table>
</div>{/block}
