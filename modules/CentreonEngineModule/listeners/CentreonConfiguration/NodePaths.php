...
use CentreonConfiguration\Events\NodePaths as NodePathsEvent;
...
class NodePaths
{
     ...
     * Execute action 
     *
     * @param \CentreonConfiguration\Events\NodePaths $event
     */
    public static function execute(NodePathsEvent $event)
    {
        $nodeId = $event->getNodeId();
        $paths = static::getPathList();
        if (!is_null($nodeId)) {
            $paths = static::getPathValues($nodeId, $paths);
        }
