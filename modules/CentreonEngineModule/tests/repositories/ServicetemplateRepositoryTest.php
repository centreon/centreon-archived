...
class ServicetemplateRepositoryTest extends DbTestCase
{
    ...
    public function testGenerate()
    {
        ...
        ServicetemplateRepository::generate($fileList, $pollerId, $this->tmpDir . '/', 'servicetemplates.cfg', $serviceMacroEvent);
        $this->assertEquals(
            array('cfg_dir' => array(
                $this->tmpDir . '/1/'
            )), $fileList
        );
        $content = file_get_contents($this->tmpDir . '/1/servicetemplates.cfg');
        /* Remove line with the generate date */
        $lines = explode("\n", $content);
        $lines = preg_grep('/^#\s+Last.*#$/', $lines, PREG_GREP_INVERT);
        $content = join("\n", $lines);
        $resultContent = file_get_contents(dirname(__DIR__) . '/data/configfiles/servicetemplate1.cfg');
