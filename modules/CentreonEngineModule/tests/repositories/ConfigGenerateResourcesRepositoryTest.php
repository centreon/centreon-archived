...
class ConfigGenerateResourcesRepositoryTest extends DbTestCase
{
    ...
    public function testGenerate()
    {
        $fileList = array();
        $pollerId = 1;
        ConfigGenerateResourcesRepository::generate($fileList, $pollerId, $this->tmpDir . '/', 'resources.cfg');
        $this->assertEquals(
            array('resource_file' => array(
                $this->tmpDir . '/1/resources.cfg'
            )), $fileList
        );
        $content = file_get_contents($this->tmpDir . '/1/resources.cfg');
        /* Remove line with the generate date */
        $lines = split("\n", $content);
        $lines = preg_grep('/^#\s+Last.*#$/', $lines, PREG_GREP_INVERT);
        $content = join("\n", $lines);
        $resultContent = file_get_contents(dirname(__DIR__) . '/data/configfiles/resource1.cfg');
