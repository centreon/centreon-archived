...
class ServicegroupRepositoryTest extends DbTestCase
{
    ...
    public function testGenerate()
    {
        $fileList = array();
        $pollerId = 1;
        ServicegroupRepository::generate($fileList, $pollerId, $this->tmpDir . '/', 'servicegroups.cfg');
        $this->assertEquals(
            array('cfg_dir' => array(
                $this->tmpDir . '/1/'
            )), $fileList
        );
        $content = file_get_contents($this->tmpDir . '/1/servicegroups.cfg');
        /* Remove line with the generate date */
        $lines = explode("\n", $content);
        $lines = preg_grep('/^#\s+Last.*#$/', $lines, PREG_GREP_INVERT);
        $content = join("\n", $lines);
        $resultContent = file_get_contents(dirname(__DIR__) . '/data/configfiles/servicegroup1.cfg');
