Cypress.Commands.add('login', () => {
  ...
  cy.fixture('users/admin')
    .as('user')
    .then((user) => {
      cy.request('POST', `${Cypress.env('DOCKER_URL')}/centreon/api/${apiVersion}/login`, {
...
Cypress.Commands.add('dockerStart', () => {
  cy.exec(`npx wait-on ${Cypress.env('DOCKER_URL')}`)
    .then(() => cy.log(`Docker Centreon started on  ${Cypress.env('DOCKER_URL')}`))
});
