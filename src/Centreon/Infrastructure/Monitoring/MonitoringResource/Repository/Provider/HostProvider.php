...
final class HostProvider extends Provider
{
    ...
    private function prepareSubQuery(
        ...
        if ($filter->getStates() && !$filter->hasState(MonitoringResourceServiceInterface::STATE_ALL)) {
            $sqlState = [];
            $sqlStateCatalog = [
                MonitoringResourceServiceInterface::STATE_UNHANDLED_PROBLEMS => "(h.state_type = '1'"
                    . " AND h.acknowledged = 0"
                    . " AND h.scheduled_downtime_depth = 0"
                    . " AND h.state != 0"
                    . " AND h.state != 4)",
                MonitoringResourceServiceInterface::STATE_RESOURCES_PROBLEMS => '(h.state != 0 AND h.state != 4)',
                MonitoringResourceServiceInterface::STATE_IN_DOWNTIME => 'h.scheduled_downtime_depth = 1',
                MonitoringResourceServiceInterface::STATE_ACKNOWLEDGED => 'h.acknowledged = 1',
            ];
